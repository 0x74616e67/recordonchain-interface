  ...
  return (
    <Listbox value={value} onChange={onChange}>
      <div className="relative sm:max-w-xs">
        ...
        <ListboxOptions
          transition
          className={`absolute z-10 mt-1 max-h-56 min-w-full overflow-auto rounded-md bg-white py-1 text-base text-sm shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none data-[closed]:data-[leave]:opacity-0 data-[leave]:transition data-[leave]:duration-100 data-[leave]:ease-in ${optionContainerClassName}`}
        >
          {options.map((option) => (
            <ListboxOption
              key={option.value}
